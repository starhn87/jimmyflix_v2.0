...
export function performanceWindows(value = new Date()) {
  const until = new Date(value)
  if (!Number.isFinite(+until)) throw new Error('Invalid --until timestamp; use an ISO 8601 date with timezone.')
  // Match the backend's four-hour aggregation buckets without overlapping weeks.
...
export async function collectPerformance({ vercel, until = new Date(), collectedAt = new Date(),
  projectId = 'prj_egb0GjkG9oeZ3AZIIrVzf7SNAjJv', hostname = 'jimmyflix.vercel.app',
}) {
  const windows = performanceWindows(until)
  const report = { collectedAt: collectedAt.toISOString(), source: 'Vercel Speed Insights',
    project: 'jimmyflix', environment: 'production', hostname, windows, queries: [], errors: [] }
  let available = new Set()
  try {
    const schema = await vercel(['metrics', 'schema', 'vercel.speed_insights'])
    if (!Array.isArray(schema) || schema.length === 0) throw new Error('Invalid schema')
    available = new Set(schema.map(({ id }) => id))
  } catch {
    report.errors.push({ error: 'Metrics schema could not be loaded; check CLI access and retry' })
  }
  const jobs = (available.size ? Object.entries(windows) : []).flatMap(([period, range]) => metrics.flatMap((metric) =>
  ...
  async function collect({ period, range, metric, aggregation }) {
    ...
    try {
      const result = await vercel(['metrics', id, '--aggregation', aggregation,
        '--project', projectId, '--prod',
        '--filter', `requestHostname:${JSON.stringify(hostname)}`, '--group-by', 'route', '--group-by', 'deviceType',
        '--since', range.since, '--until', range.until, '--granularity', '1h', '--limit', '100'])
      if (!Array.isArray(result.summary) || result.summary.some((row) => typeof row.route !== 'string' || typeof row.deviceType !== 'string')) throw new Error('Invalid metrics summary')
  ...
  const delta = (row, metric) => {
    ...
    if ((row.current[`${metric}_count`] ?? 0) < MIN_COMPARISON_SAMPLES || (row.previous[`${metric}_count`] ?? 0) < MIN_COMPARISON_SAMPLES) return '—'
  ...
  const md = [
    ...
    '## 직전 7일 대비 p75 변화', '',
    `기간별 표본이 각각 ${MIN_COMPARISON_SAMPLES}개 이상인 지표만 차이를 표시합니다. 이는 보고서의 최소 표본 정책이며 통계적 유의성을 보장하지 않습니다.`, '',
