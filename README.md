...
| OTT 서비스별 작품 선택 | 다음 회차와 최신 시즌 에피소드 | 찜·봤어요·관심 없음 상태 관리 | 시스템 기본 테마와 수동 전환 |
| 일간 주제·주간 시대 및 포맷 특집 | 시청 플랫폼, 컬렉션, 관련 추천작 | 조건에 맞춘 세 작품 추천 | 언어와 독립된 시청 국가 설정 |
...
목록 카드에서 바로 작품을 찜하고, 상세 화면에서는 `찜`, `봤어요`, `관심 없음` 중 하나로 상태를 정할 수 있습니다. 저장한 작품은 보관함에서 상태별로 모아보며 한국어와 영어 화면 사이에서도 그대로 유지됩니다.

무엇을 볼지 정하기 어렵다면 보관함의 조건 탐색에서 영화·TV, 장르, 최대 러닝타임, OTT, 최소 평점, 정렬 기준을 조합할 수 있습니다. 조건에 맞는 전체 결과와 함께 먼저 고른 세 작품을 보여주며, 이미 본 작품은 후보에서 제외할 수 있습니다.

## 모바일 화면

모바일 상세는 포스터를 화면 너비로 크게 보여주고 그 아래에 제목과 핵심 정보를 배치합니다. 헤더의 설정 메뉴에서 언어, 테마, 시청 국가를 함께 바꿀 수 있으며, 선택한 값은 다음 방문에도 유지됩니다.
...
| 상태 | 검색 조건은 URL, 보관함·최근 본 작품·언어·테마·시청 국가는 브라우저에 저장 |
