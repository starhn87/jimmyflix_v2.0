...
test('external media loads directly without exposing an unavailable image optimizer', async ({ page, isMobile }) => {
  ...
  await page.goto('/en')
  const hero = page.locator('section[aria-labelledby="featured-title"] img')
  const card = page.getByAltText('Movie 1 poster', { exact: true }).first()
  ...
  await expect(page.locator('head link[rel="preload"][as="image"]')).toHaveCount(1)
  await expect(page.locator('main picture img')).toHaveAttribute('fetchpriority', 'low')
  ...
  await expect.poll(() => actor.evaluate((image) => image.currentSrc)).toContain('/h632/')
  ...
  await expectCdnImage(portrait)
  await expect.poll(() => portrait.evaluate((image) => image.currentSrc)).toContain('/original/')

  await page.goto('/en/movies/996')
  const placeholder = page.getByAltText('Movie 996 poster', { exact: true }).first()
  await expect.poll(() => placeholder.evaluate((image) => new URL(image.src).pathname)).toBe('/images/defaultPoster.png')
