...
globalThis.fetch = async (input, options) => {
  ...
  if (parts[0] === 'person' && Number.isFinite(id)) return Response.json({
    id, name: `Actor ${id - 999}`, profile_path: poster(id),
    biography: id === 1002 ? '' : id === 1001 && url.searchParams.get('language') === 'ko-KR'
      ? '' : 'An actor with an extensive filmography.',
    known_for_department: 'Acting', birthday: '1980-01-01', deathday: null,
    place_of_birth: 'Seoul', also_known_as: [],
    combined_credits: { cast: [item(1), item(2)], crew: [] },
  })
