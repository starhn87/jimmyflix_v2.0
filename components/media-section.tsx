  ...
  if (section.items.length === 0) {
    return (
      <section className="px-4 sm:px-6 lg:px-10" aria-labelledby={`${section.id}-title`}>
        <h2 id={`${section.id}-title`} className="text-xl font-semibold text-white sm:text-2xl">
          {section.title}
        </h2>
        <p className="mt-4 rounded-2xl border border-white/8 bg-white/4 p-6 text-sm text-slate-400">
