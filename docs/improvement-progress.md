...
| J Ticket·테마 전환 | 구현·원격 검증 완료 | 배경 없는 티켓 favicon, 헤더 토글, 선택 저장·탭 동기화, 전 화면 다크·라이트 색상 |
| 카드 정보 간소화 | 구현·원격 검증 완료 | 없는 평점 배지 생략, 목록에서 이미 구분한 영화·TV 문구 제거, 실제 검색·상세 데이터 검증 |
| 통합 검색 | 구현·원격 검증 완료 | Search 메뉴·중복 입력창 제거, 제목·배우·주제 키워드 검색, 모바일 펼침, 결과 중복 제거·부분 실패 처리 |
| 한국어·영어 전환 | 로컬 구현·검증 완료 | 언어별 URL·TMDB 현지화·UI 사전·선택 저장·기존 URL 리다이렉트 |
| Velvet Violet 브랜드 컬러 | 로컬 적용·검증 완료 | 보라→핑크 J 티켓, 다크·라이트 토큰, 배경 광원과 상호작용 색상 |
...
- 카드 평점을 포스터 위의 배지로 바꾸고 소수점 한 자리로 정리했다. 평점이 없으면 카드·홈 대표 콘텐츠·상세 화면 모두 배지를 생략한다. 제목 아래에는 연도만 표시하며, 목록에서 이미 구분한 영화·TV 유형은 반복하지 않는다.
...
- 전체 App Router를 `app/[locale]` 아래로 옮기고 영어 `/en`, 한국어 `/ko` 경로를 정적 생성한다. 언어가 없는 기존 URL은 저장된 선택과 브라우저 언어에 맞춰 리다이렉트한다.
- 헤더 언어 버튼은 검색어·트렌드 기간을 포함한 현재 URL을 유지한다. 선택은 `jimmyflix-locale-v1` 쿠키에 저장되며 새로고침과 기존 URL 진입에도 적용된다.
- 내비게이션, 검색, 목록, 상세, 탭, 오류, 빈 상태, 404, 로딩, 메타데이터를 서버 전용 사전으로 현지화했다. TMDB의 `language`도 `en-US` 또는 `ko-KR`로 바꿔 제목·줄거리·장르·검색 결과가 UI 언어를 따른다.
- 320px 헤더에서 영어의 긴 메뉴와 검색·언어·테마 버튼이 겹치지 않도록 메뉴 글자와 간격을 조정하면서 세 제어 버튼은 44×44px 조작 영역을 유지했다.
- 브랜드 컬러는 Ticket Cyan, Marquee Amber, Velvet Violet 세 방향을 비교한 뒤 Velvet Violet으로 확정했다. 다크 Canvas `#0B0912`·Primary `#A78BFA`, 라이트 Canvas `#FAF8FF`·Primary `#6D28D9`를 적용하고 J 티켓은 `#A78BFA → #EC4899` 그라데이션으로 통일했다.
...
- 현지화 빌드: `npm run lint`, `npm run typecheck`, `npm run build` 통과. `/en`, `/ko`, `/en/tvs`, `/ko/tvs`가 언어별 정적 경로로 생성되고 검색·트렌드·상세의 동적 경로가 `[locale]` 아래에 포함되는지 확인
- 언어 감지: 한국어 `Accept-Language`는 `/ko`, 저장된 영어 쿠키는 `/en`, 기존 `/search?q=time%20travel`과 한국어 쿠키 조합은 `/ko/search?q=time+travel`로 리다이렉트되는지 확인
- 언어 전환: `/ko/search?q=송강호`에서 영어로 전환한 뒤 `/en/search?q=송강호`와 `<html lang="en">`을 유지하고 쿠키가 `en`으로 바뀌는지 확인. 트렌드의 `window=week`도 같은 방식으로 보존되는지 확인
- 320px 현지화 헤더: 영어·한국어 메뉴, 검색, 언어, 테마 제어가 겹치지 않고 가로 넘침이 0px이며 검색·언어·테마가 각각 44×44px 조작 영역을 유지하는지 확인
- 현지화 오류 경로: `/ko/movies/not-a-number`의 한국어 404와 `/ko/movies/550`의 한국어 오류 경계에서 올바른 제목·복귀 경로·재시도 문구, 가로 넘침 0px, Next 오류 오버레이 없음 확인
- Velvet Violet 대비: 다크의 본문 18.92:1·가장 옅은 보조문자 6.75:1·버튼 7.10:1, 라이트의 본문 16.84:1·가장 옅은 보조문자 5.56:1·버튼 7.10:1로 WCAG AA 이상 확인
- Velvet Violet 브라우저: 1440px 다크·라이트에서 Canvas·Surface·Accent·Action과 `theme-color`가 선택 테마에 맞게 계산되고, 테마 전환 후 가로 넘침·오류 오버레이·콘솔 오류가 없는지 확인
- Velvet Violet 모바일·아이콘: 320px에서 메뉴와 44×44px 제어가 겹치지 않고, SVG와 16·24·32·48·64px ICO가 보라→핑크 티켓과 투명 J 형태를 유지하는지 확인
- Vercel 프리뷰 `dpl_EtZ6dutRGww5e4jyTxSd5XTiYSaZ`: 커밋 `8053663`을 Node.js 24로 빌드하고 실제 TMDB 데이터로 한국어 홈 81개 상세 링크, 한국어 상세 제목·줄거리·장르·탭·예고편 이름을 확인
- 프리뷰 현지화 검색·테마: `송강호` 검색에서 관련 인물과 영화·TV 62개 결과를 표시하고 영어 전환 후 검색어를 유지했다. 320px 실제 포스터 화면의 다크·라이트 Velvet Violet, 44px 헤더 제어, 가로 넘침 0px와 최근 15분 런타임 오류 0건을 확인
